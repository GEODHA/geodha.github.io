...
// TODO(i18n): strings are English-only for now (nav/footer are translated).

import { Link } from 'react-router-dom';
import {
  Mail, MessageCircle, Newspaper, Recycle, HandHeart, ArrowRight,
} from 'lucide-react';
...
const APP_SLIDES: { src: string; title: string; sub: string }[] = [
  { src: shot1, title: 'Quick reporting',      sub: 'Photo + GPS. 30 seconds.' },
  { src: shot2, title: 'Direct reporting',     sub: 'Straight to the right authority.' },
  { src: shot3, title: 'Public data',          sub: 'Every report on a live, open map.' },
  { src: shot4, title: 'Escalation guidance',  sub: 'No response? We show the next step.' },
];
...
const STATS = [
  { value: '150+', label: 'App users',               color: 'text-secondary' },
  { value: '100+', label: 'Reports filed',           color: 'text-primary' },
  { value: '~10',  label: 'Documented case studies', color: 'text-accent' },
];
...
function MarqueeBanner() {
  const words = (
    <span className="inline-block font-mono text-xs font-bold uppercase tracking-[0.14em] px-2">
      <span className="text-secondary px-4">Data for Action.</span>
      <span className="text-primary px-4">Data for Awareness.</span>
      <span className="text-accent px-4">Data for Accountability.</span>
    </span>
  );
  return (
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-secondary border-b-[3px] border-ink py-2 px-4 text-center">
        <p className="text-xs sm:text-sm font-bold text-ink">
          📱 The GEODHA reporting app is now available!{' '}
          <a href={PLAY_STORE_URL} target="_blank" rel="noopener noreferrer" className="underline underline-offset-2 font-extrabold">Google Play</a>
          {' '}·{' '}
          <a href={IOS_APP_URL} target="_blank" rel="noopener noreferrer" className="underline underline-offset-2 font-extrabold">iOS Beta</a>
        </p>
      </div>
      ...
      <section className="relative bg-paper border-b-[3px] border-ink py-14 sm:py-16 text-center px-4 overflow-hidden">
        ...
        <div className="relative">
          ...
          <h1 className="mx-auto max-w-4xl font-black tracking-[-0.035em] leading-[1.02] text-[38px] sm:text-[56px] lg:text-[64px]">
            Clean, <span className="text-primary">Liveable</span> and{' '}
            <span className="text-accent">Sustainable</span> Cities.
          </h1>
        </div>
      </section>
      ...
      <section className="bg-tint-yellow py-10 sm:py-12 border-b-[3px] border-ink">
        <div className="container px-4 sm:px-6 lg:px-8">
          <SectionHead title="The Problem" caption="It's on every street." />
          <div className="max-w-3xl mx-auto">
            <SimpleCarousel slides={PROBLEM_SLIDES} aspect="aspect-[16/10]" fit="cover" />
          </div>
          <p className="mono-label text-center mt-8">
            Real images collected through the GEODHA reporting app
          </p>
        </div>
      </section>
      ...
      <section className="bg-paper py-10 sm:py-12">
        <div className="container px-4 sm:px-6 lg:px-8">
          <SectionHead kicker="Our Solutions · 01" title="The Reporting App" />
        ...
        <div className="container px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-5 gap-8 items-center">
            <div className="lg:col-span-2">
              <p className="mono-label text-accent mb-1.5">Our Solutions · 02</p>
              <h2 className="text-3xl sm:text-4xl font-black tracking-[-0.03em] leading-[1.05]">
                The whole city's waste, mapped.
              </h2>
              <p className="mt-3 text-foreground/70 font-medium leading-relaxed max-w-sm">
                Every ward's garbage situation — live, open and public. Tap any
                ward for details and recommended actions.
              </p>
            </div>
            ...
            {[
              {
                ...
                title: 'Blog',
                body: 'Waste management and circular economy ideas.',
              },
              {
                Icon: Recycle, tile: 'bg-primary', card: 'bg-tint-green', href: '/guide',
                title: 'Segregate right',
                body: 'Learn how to segregate and dispose waste correctly.',
              },
              {
                Icon: HandHeart, tile: 'bg-secondary', card: 'bg-tint-yellow', href: '/volunteer',
                title: 'Volunteer',
                body: 'Join cleanups and check out upcoming events.',
              },
      ...
      <section className="bg-primary text-white py-12 sm:py-14 text-center px-4">
        <h2 className="text-3xl sm:text-5xl font-black tracking-[-0.03em]">Join us in our mission</h2>
        <p className="mt-3 text-lg font-medium text-white/85 max-w-xl mx-auto">
          Reach out to collaborate, volunteer, or bring GEODHA to your city.
        </p>
